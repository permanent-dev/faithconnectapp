...
function Dashnav() {
    ...
    const navlinks = [
        ...
        { id: 3, name: "books", icon: <LibraryBigIcon size={45} />, link: '/dashboard/books' },
        { id: 4, name: "blogs", icon: <Logs size={45} />, link: '/dashboard/blogs' }
    ];
    ...
    return (
        <div>
            <div className={`flex flex-col space-y-20 bg-[white] rounded-l-[1.8rem] 
                ${isOpen ? 'w-77 ' : 'space-y-26 w-77'}`}
            >
                <div className={`${isOpen ? '' : 'bg-[#ee9922] px-6 py-2 w-77 '} rounded-tl-[1.8rem] flex gap-13`}>
                ...
                <ul className='flex flex-col gap-12'>
                    ...
                    {navlinks.map(nav => (
                        <li key={nav.id} className=' capitalize font-bold text-[1.7rem]' >
                            ...
                            <NavLink to={nav.link} className={({ isActive }) => `flex gap-4 px-3 py-2 rounded-r-full transition-colors duration-300 w-77 ${isActive ? "bg-[#ffa500] text-white" : "hover:bg-orange-300"}`
                            }>
